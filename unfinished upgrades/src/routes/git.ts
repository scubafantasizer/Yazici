...
function runGit(cmd: string, cwd?: string): { output: string; success: boolean } {
  try {
...
router.get('/api/projects/:projectId/git/status', (req, res) => {
  ...
  const modified: string[] = [], staged: string[] = [], untracked: string[] = [], deleted: string[] = [];
...
router.post('/api/projects/:projectId/git/commit', (req, res) => {
  const { message, stageAll = true } = req.body as { message: string; stageAll?: boolean };
  if (!message) { res.status(400).json({ error: 'message required' }); return; }
  if (stageAll) runGit('git add -A');
